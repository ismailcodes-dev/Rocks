...
class DatabaseManager:
    ...
    def _init_sync(self):
        # Initialize economy.db (Unchanged)
    ...
    # ... (get_user_data and update_user_data are unchanged) ...
    ...
    def _get_shop_items_sync(self, guild_id, order_by, limit=None):
        with sqlite3.connect(self.shop_db_path) as con:
            con.row_factory = sqlite3.Row
            cur = con.cursor()
            if order_by not in ["purchase_count", "upload_timestamp", "item_name"]:
    ...
    # --- NEW: Function to get top users for the leaderboard ---
